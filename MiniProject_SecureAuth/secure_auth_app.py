...
# ----------------------------------------------------------------------
# Password policy (balanced usability)
# ----------------------------------------------------------------------
...
def estimate_entropy(password: str) -> float:
    """
    Rough Shannon-style entropy estimate based on character set size and length.
    This is not cryptographically exact but is sufficient for policy decisions.
    """
    if not password:
        return 0.0
    ...
    if pool == 0:
        # Fallback if somehow none of the above match
        pool = 10
...
def password_is_acceptable(password: str) -> tuple[bool, str, float]:
    ...
    # Example threshold; may be adjusted if required.
    if entropy < 50.0:
